...
export default function DashboardModules({ data }) {
  ...
  const setLivros = async (url, name, description) => {
    try {
      await setDoc(doc(db, "livros", uuidv4()), {
        name: name,
        url: url,
  ...
  return (
    <section className='w-full max-w-[1280px] px-8 py-16'>
      <div className='flex items-center gap-3'>
        <h1 className=' text-2xl my-2'>Livros</h1>
          ...
          <div key={k} className='flex items-center justify-between gap-10 p-4 w-full'>
            ...
            <div className='flex gap-8 items-center'>
              <img src={v.url && v.url.includes("https://imgur.com") ?
                v.url.replace("https://imgur.com", "https://i.imgur.com").concat(".png") : v.url} alt={v.name} className='w-20 h-20' />
            ...
            < div className='flex gap-4' >
              ...
              <button className=' bg-red-600 hover:bg-red-800 p-2 rounded'><FaTrash color='white' onClick={async () => {
                await deleteDoc(doc(db, "livros", v.id));
              }} /></button>
            </div>
          </div>
        ))
